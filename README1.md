...
| 4 | Tech Stack Plan | ✅ |
| 5 | Product Workflow | ✅ |
| 6 | 2-Minute Strategy Video | 🔄 Pending |
...
|---------|----------------------|-----------------|
| Extreme Heat | 15-20 days/year (Mar–Jun) | ✅ High — relatable, emotional |
| Severe AQI | 30-40 days/year (Oct–Jan) | ✅ High — NCR-specific, worsening |
| Heavy Rain | 50-60 days/year (Jul–Sep) | ✅ High — visible, disruptive |
| Zone Restriction | 10-15 events/year | ⚠️ Medium — harder to demo without live news feed |

Adding a 5th trigger (flooding) and advanced news NLP is a Phase 2 move. Phase 1 needs a clean, believable set of 4 triggers that judges can reason about in 2 minutes.
...
Phase 1 demonstrates AI **thinking**, not AI **production systems**. Judges want to see that AI makes the product smarter, not that you have a full ML pipeline.
...
This is a real differentiator. Judges will notice the fairness and transparency story.
...
Hackathons like DEVTrails often introduce surprise compliance or market scenarios near Phase 1 deadlines. GigShield is built to absorb these shocks without rewriting core architecture.

### Pre-Built Resilience Architecture

Three design choices that make GigShield crash-proof:

| Design Choice | What It Does | Crash Scenario It Solves |
|---|---|---|
| **Feature Flag System** | Each trigger, tier, and multiplier is a config value — not hardcoded | New trigger or coverage change = update config, not code |
| **Modular Trigger Engine** | Reads from `backend/data/triggers.js`. Adding/removing triggers requires one config entry | Must add "platform outage" trigger = add one line to config |
| **Dynamic Pricing Config** | `Base_Premium`, `Zone_Risk`, `Season_Factor` are variables in `backend/data/pricing.js` | Premium cap imposed = change one floor value, everything recalculates |

### 24-Hour Crash Playbook

If a market crash scenario is announced within 24 hours of the Phase 1 deadline:

| Step | Action | Time |
|------|--------|------|
| 1 | Read the crash scenario — identify which constraint changes | 0–30 min |
| 2 | Map the change to one zone: pricing, coverage, or compliance | 30–60 min |
| 3 | If it conflicts with Phase 1 rules — flag as Phase 2 upgrade, do not restructure | 60–90 min |
| 4 | Pricing shock → update `backend/data/pricing.js` (Base_Premium floor, tier caps) | 90–120 min |
| 5 | New trigger needed → add entry to `backend/data/triggers.js`, engine handles it | 90–120 min |
| 6 | Update this section in README1.md with the specific adaptation | 2–4 hrs |
| 7 | Re-record or re-script the 2-minute video if the crash changes the core pitch | 4–5 hrs |

### Specific Crash Adaptations

| Crash Type | Phase 1 Impact | Adaptation | Enabler |
|---|---|---|---|
| **Premium cap imposed** (e.g., max ₹40/week) | ₹49 floor breaks | Change `Base_Premium` to ₹30 in pricing.js | Config-based pricing |
| **New trigger required** (e.g., platform outage) | Out of scope for Phase 1 | Add one entry to `triggers.js` config | Modular trigger engine |
| **Coverage expansion mandate** (e.g., include accidents) | Contradicts Phase 1 rules | Decline — flag for Phase 2, document rationale | Clear scope boundary |
| **Mass claims event** (all partners file at once) | Tests payout pipeline | Fraud engine runs per claim; async queue handles volume | Payout queue architecture |
| **Platform earnings data unavailable** | Declared earnings fallback needed | Use historical district average from `zones.js` | Pre-loaded zone risk data |

### Phase 1 Scope Protection

If a crash scenario tries to push Phase 1 into Phase 2 territory — we do not restructure. We document, we adapt the config, and we note it as a Phase 2 priority. Phase 1 survives on focus.
...
|------|------|----------------|
| TBD | Full Stack Lead | Frontend (React PWA), API development, deployment |
| TBD | Backend & Data | Express.js APIs, SQLite DB, trigger engine |
| TBD | AI/ML | Pricing model, fraud detection, explainability |
| TBD | Product & Design | UX research, UI design, video, pitch |
...
| 🎬 2-Minute Strategy Video | 🔄 Pending |
| 🌐 Live Demo (Phase 2) | 🔄 Pending |

---

<div align="center">

### *Built for India's 10 million+ delivery heroes*
